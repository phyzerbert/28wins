...
class IndexController extends Controller
{
    ...
    public function transfer_credit(Request $request) {
        $transfer_flag = Setting::find(1)->transfer_flag;
        if(!$transfer_flag) return back()->withErrors(['transfer_flag' => __('error.transfer_flag_error')]);
        $request->validate([
            'amount' => 'required|min:0',
        ]);
        $user = Auth::user();
        if($user->score < $request->amount) {
            return back()->withErrors(['balance' => __('words.insufficient_balance')]);
        }
        $agent = $user->agent;
