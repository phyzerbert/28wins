...
class GameController extends Controller
{
    ...
    public function open($id) {
        ...
        switch ($game->name) {
            ...
            case 'xe88':
                return back()->withErrors(['error' => __('error.contact_with_administrator')]);
            ...
            case 'goldenf_lottery':
                $game_account = $this->goldenf_lottery();
                break;
                
            case 'abs':
                $game_account = $this->abs();
                break;
    ...
    public function deposit(Request $request) {
        ...
        switch ($game->name) {
            ...
            case 'goldenf_lottery':
                $result = $this->goldenf_lottery_deposit($amount);
                break;
            
            case 'abs':
                $result = $this->abs_deposit($amount);
                break;
    ...
    public function withdraw(Request $request) {
        ...
        switch ($game->name) {
            ...
            case 'goldenf_lottery':
                $result = $this->goldenf_lottery_withdraw($amount);
                break;
            
            case 'abs':
                $result = $this->abs_withdraw($amount);
                break;
    ...
    public function total_get_balance($id) {
        ...
        switch ($game->name) {
            ...
            case 'goldenf_lottery':
                $result = $this->goldenf_lottery_get_balance();
                break;
            
            case 'abs':
                $result = $this->abs_get_balance();
                break;
    ...
    public function play($id) {
        ...
        switch ($game->name) {
            ...
            case 'goldenf_lottery':
                return redirect(route('game.goldenf_lottery.list'));
                break;            
            
            case 'abs':
                return redirect(route('game.abs.play'));
                break;
    ...
    public function goldenf_lottery(){
        $user = Auth::user();
        $game = Game::where('name', 'goldenf_lottery')->first();
        $domain = $game->domain;
        $secret_key = $game->api_key;
        $operator_token = $game->token;
        $auth_user = Auth::user();
    ...
    public function goldenf_lottery_deposit($amount) {
        $game = Game::where('name', 'goldenf_lottery')->first();
        $domain = $game->domain;
        $secret_key = $game->api_key;
        $operator_token = $game->token;
    ...
    public function goldenf_lottery_withdraw($amount) {
        $game = Game::where('name', 'goldenf_lottery')->first();
        $domain = $game->domain;
        $secret_key = $game->api_key;
        $operator_token = $game->token;
    ...
    public function goldenf_lottery_get_balance(){
        $game = Game::where('name', 'goldenf_lottery')->first();
        $domain = $game->domain;
        $secret_key = $game->api_key;
        $operator_token = $game->token;
    ...
    public function goldenf_lottery_list() {
        config(['site.page' => 'lottery']);
        config(['site.wap_footer' => 'game']);
        $game = Game::where('name', 'goldenf_lottery')->first();
        if(is_Mobile()) {
            return view('wap.game.goldenf_lottery', compact('game'));
        } else {
            return view('web.game.goldenf_lottery', compact('game'));
        }
    }

    public function goldenf_lottery_play(Request $request) {
        $game_code = $request->code;
        $game = Game::where('name', 'goldenf_lottery')->first();
        $deposit_status = $this->total_deposit($game->id);
        if($deposit_status != 'success') {return back()->withErrors(['error' => __('error.something_went_wrong')]);}
    ...
    public function abs() {
        ...
        $game_account = GameUser::where('user_id', $auth_user->id)->where('game_id', $game->id)->first();
        if (!$game_account) {
            $username = "26W" . getRandom(4) . $auth_user->id;
            $creat_account = 1;
        } else {
            $username = $game_account->username;
        }
        ...
        if($result->Success == true){
            ...
            session(['abs_login_url' => $result->LoginURL]);
        } else {
            return 'error';
        }
        $post_data = [   
    ...
    public function abs_deposit($amount) {
        $game = Game::where('name', 'abs')->first();
        $domain = $game->domain;
        $parent_id = $game->api_key;
        $hash_code = $game->token;
        ...
        if($result->Success == true) {            
            ...
            $game_account->increment('balance', $result->Amount);
            return 'success';
        } else {
            return 'error';
        }
    ...
    public function abs_withdraw($amount) {
        $game = Game::where('name', 'abs')->first();
        $domain = $game->domain;
        $parent_id = $game->api_key;
        $hash_code = $game->token;
        ...
        if($result->Success == true) {            
            ...
            $game_account->decrement('balance', $result->Amount);
            return 'success';
        } else {
            return 'error';
        }
    ...
    public function abs_get_balance(){
        $game = Game::where('name', 'abs')->first();
        $domain = $game->domain;
        $parent_id = $game->api_key;
        $hash_code = $game->token;
    ...
    public function abs_play(){
        $login_url = session('abs_login_url');
